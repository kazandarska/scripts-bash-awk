...
* Implement conditional logic in `awk` (`if-else if-else`): Created advanced internal conditions to dynamically process data based on specific criteria.
* Utilize loops in `awk` (`for` loops): Learned how to iterate efficiently through row fields (columns) using the `NF` variable to automate calculations and the difference in usage between NR and NF.
